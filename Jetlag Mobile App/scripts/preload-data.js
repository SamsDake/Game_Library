...
async function main() {
  ...
  if (ADMIN_LEVELS.length) {
    ...
    try { manifest.countryOutlines = JSON.parse(fs.readFileSync(path.join(OUT_DIR, 'manifest.json'), 'utf8')).countryOutlines; } catch { /* preserve nothing if old manifest is missing */ }
  }
